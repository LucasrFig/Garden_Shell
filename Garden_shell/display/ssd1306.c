...
void ssd1306_draw_full_image(ssd1306_t *ssd, const uint8_t *bitmap)
{
...
void ssd1306_draw_16x16_image(ssd1306_t *ssd, const uint8_t *bitmap, uint8_t x, uint8_t y) {
  for (int i = 0; i < 16; i++) {          // Linhas (altura da imagem)
    for (int j = 0; j < 16; j++) {      // Colunas (largura da imagem)
        // Calcula o byte e o bit correspondente ao pixel
        uint8_t byte_index = i * 2 + (j / 8);  // Cada linha tem 2 bytes (16 pixels)
...
void ssd1306_draw_32x32_image(ssd1306_t *ssd, const uint8_t *bitmap, uint8_t x, uint8_t y) {
  for (int i = 0; i < 32; i++) {          // Linhas (altura da imagem)
      for (int j = 0; j < 32; j++) {      // Colunas (largura da imagem)
          // Calcula o byte e o bit correspondente ao pixel
          uint8_t byte_index = i * 4 + (j / 8);  // Cada linha tem 4 bytes (32 pixels)
          uint8_t bit_index = 7 - (j % 8);       // Inverte a ordem dos bits dentro do byte
          uint8_t pixel = (bitmap[byte_index] >> bit_index) & 0x01; // Extrai o bit
...
void ssd1306_draw_40x40_image(ssd1306_t *ssd, const uint8_t *bitmap, uint8_t x, uint8_t y) {
  for (int i = 0; i < 40; i++) {          // Linhas (altura da imagem)
      for (int j = 0; j < 40; j++) {      // Colunas (largura da imagem)
          // Calcula o byte e o bit correspondente ao pixel
          uint8_t byte_index = i * 5 + (j / 8);  // Cada linha tem 5 bytes (40 pixels)
          uint8_t bit_index = 7 - (j % 8);       // Inverte a ordem dos bits dentro do byte
          uint8_t pixel = (bitmap[byte_index] >> bit_index) & 0x01; // Extrai o bit
...
void ssd1306_draw_48x48_image(ssd1306_t *ssd, const uint8_t *bitmap, uint8_t x, uint8_t y) {
  for (int i = 0; i < 48; i++) {          // Linhas (altura da imagem)
      for (int j = 0; j < 48; j++) {      // Colunas (largura da imagem)
          // Calcula o byte e o bit correspondente ao pixel
          uint8_t byte_index = i * 6 + (j / 8);  // Cada linha tem 6 bytes (48 pixels)
          uint8_t bit_index = 7 - (j % 8);       // Inverte a ordem dos bits dentro do byte
          uint8_t pixel = (bitmap[byte_index] >> bit_index) & 0x01; // Extrai o bit
...
void ssd1306_draw_56x56_image(ssd1306_t *ssd, const uint8_t *bitmap, uint8_t x, uint8_t y) {
  for (int i = 0; i < 56; i++) {          // Linhas (altura da imagem)
      for (int j = 0; j < 56; j++) {      // Colunas (largura da imagem)
          // Calcula o byte e o bit correspondente ao pixel
          uint8_t byte_index = i * 7 + (j / 8);  // Cada linha tem 7 bytes (56 pixels)
          uint8_t bit_index = 7 - (j % 8);       // Inverte a ordem dos bits dentro do byte
          uint8_t pixel = (bitmap[byte_index] >> bit_index) & 0x01; // Extrai o bit
...
void ssd1306_draw_64x64_image(ssd1306_t *ssd, const uint8_t *bitmap, uint8_t x, uint8_t y) {
  for (int i = 0; i < 64; i++) {          // Linhas (altura da imagem)
      for (int j = 0; j < 64; j++) {      // Colunas (largura da imagem)
          // Calcula o byte e o bit correspondente ao pixel
          uint8_t byte_index = i * 8 + (j / 8);  // Cada linha tem 8 bytes (64 pixels)
          uint8_t bit_index = 7 - (j % 8);       // Inverte a ordem dos bits dentro do byte
          uint8_t pixel = (bitmap[byte_index] >> bit_index) & 0x01; // Extrai o bit
